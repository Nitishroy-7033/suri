...
const TOOLS = {
  ...
  holo_show:          { icon: "holo",   run: "Projecting a hologram",     done: "Hologram up",           args: (a) => [a.name && `“${a.name}”`] },
  holo_view:          { icon: "holo",   run: "Moving the hologram",       done: "Moved the hologram",    args: (a) => [a.action?.replaceAll("_", " ")] },
  holo_highlight:     { icon: "holo",   run: "Highlighting a part",       done: "Highlighted",           args: (a) => [a.part && `“${a.part}”`] },
  holo_panel:         { icon: "holo",   run: "Arranging the workshop",    done: "Workshop updated",      args: (a) => [a.action, a.target] },
  holo_status:        { icon: "eye",    run: "Looking at the workshop",   done: "Looked at the workshop" },
  fs_find:            { icon: "folder", run: "Searching your files",      done: "Searched your files",   args: (a) => [a.name && `“${a.name}”`] },
  fs_ask:             { icon: "folder", run: "Reading a file",            done: "Read a file",           args: (a) => [a.path?.split(/[\\/]/).pop()] },
  fs_delete:          { icon: "trash",  run: "Asking to delete",          done: "Waiting for your OK",   args: (a) => [a.paths?.length && `${a.paths.length} item${a.paths.length > 1 ? "s" : ""}`] },
  pc_control:         { icon: "sliders", run: "Adjusting the PC",         done: "Adjusted the PC",       args: (a) => [a.control, a.value ?? a.action] },
  pc_status:          { icon: "sliders", run: "Checking the PC",          done: "Checked the PC" },
  forge_find:         { icon: "holo",   run: "Searching for a model",     done: "Searched for a model",  args: (a) => [a.query && `“${a.query}”`] },
  forge_build:        { icon: "holo",   run: "Building a model",          done: "Model build started",   args: (a) => [a.description && `“${a.description}”`] },
  forge_job:          { icon: "holo",   run: "Checking the forge",        done: "Checked the forge",     args: (a) => [a.action] },
};
